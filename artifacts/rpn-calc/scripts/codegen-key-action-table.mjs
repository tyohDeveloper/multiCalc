...
// --- Read suppression flags from suppressionFlags.ts at build time ---
// Parse the TypeScript source as text; no runtime import needed.
const SUPPRESSION_FLAGS_PATH = join(ROOT, "src/state/suppressionFlags.ts");
const suppressionFlagsSource = readFileSync(SUPPRESSION_FLAGS_PATH, "utf8");

function readBoolFlag(source, name) {
  const match = source.match(new RegExp(`export\\s+const\\s+${name}\\s*=\\s*(true|false)`));
  if (!match) {
    console.error(`[codegen] ERROR: Flag "${name}" not found in suppressionFlags.ts — expected "export const ${name} = true|false"`);
    process.exit(1);
  }
  return match[1] === "true";
}

const NO_GRAPHICS   = readBoolFlag(suppressionFlagsSource, "NO_GRAPHICS");
const NO_PROGRAMMING = readBoolFlag(suppressionFlagsSource, "NO_PROGRAMMING");
const NO_MATRIX     = readBoolFlag(suppressionFlagsSource, "NO_MATRIX");
...
for (const section of keyData.sections) {
  for (const row of section.rows) {
    row.keys.forEach((rawKey, idx) => {
      ...
      // unshifted action
      const unshiftedAction = resolveActionLiteral(key.op, context);
...
const layoutLines = [
  ...
  " * based on the active flags in src/state/suppressionFlags.ts.",
  " */",
  "",
