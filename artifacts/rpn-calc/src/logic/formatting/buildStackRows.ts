...
export function buildStackRows(state: CalcState): StackRowVM[] {
  ...
  const fmt = (v: number) => formatStackValue(v, displayMode, displayPrecision);
  return [
    ...
    { label: t("stack-x"), value: entry.isActive ? formatEntryBuffer(entry.buffer) : fmt(stack[0]), isEntry: entry.isActive },
  ];
}
